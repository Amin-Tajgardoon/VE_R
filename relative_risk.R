...
calcRelativeRisk <- function(mymatrix,alpha=0.05,referencerow=2)
{
  ...
  for (i in 1:numrow)
  {
    ...
    if (i != referencerow)
    {
      ...
      print(paste("category =", rowname, ", relative risk = ",1-relativeRisk))
      ...
      print(paste("category =", rowname, ", ", confidenceLevel,
                  "% confidence interval = [",1-lowervalue,",",1-uppervalue,"]"))
    }
  }
}

mymatrix <- matrix(c(49,5054,75,2474),nrow=2,byrow=TRUE)
